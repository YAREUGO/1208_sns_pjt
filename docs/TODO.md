...
- [x] Supabase 데이터베이스 마이그레이션
  - [x] `db.sql` 파일을 Supabase에 적용 (마이그레이션 파일 생성 완료)
  - [ ] 테이블 생성 확인 (users, posts, likes, comments, follows) - 수동 작업 필요
  - [ ] Views 및 Triggers 확인 - 수동 작업 필요
- [ ] Supabase Storage 버킷 생성
  - [ ] `posts` 버킷 생성 (공개 읽기) - 수동 작업 필요
  - [ ] 업로드 정책 설정 - 수동 작업 필요
- [x] TypeScript 타입 정의
...
- [x] `app/api/likes/route.ts`
  ...
  - [x] DELETE: 좋아요 제거
  - [x] 인증 검증 (Clerk)
- [x] `app/api/likes/[postId]/route.ts`
  - [x] GET: 좋아요 상태 조회
...
- [x] `components/comment/CommentList.tsx`
  ...
  - [x] 상세 모달: 전체 댓글 + 스크롤 (showAll prop)
  - [x] 삭제 버튼 (본인만 표시)
...
- [x] `app/api/comments/route.ts`
  ...
  - [x] DELETE: 댓글 삭제 (본인만)
  - [x] 인증 검증 (Clerk)
- [x] `app/api/comments/[postId]/route.ts`
  - [x] GET: 댓글 목록 조회
